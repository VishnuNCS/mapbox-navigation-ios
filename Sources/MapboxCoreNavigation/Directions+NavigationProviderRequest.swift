...
extension Directions: NavigationProvider {
    
    @discardableResult public func requestRoutes(options: RouteOptions, completionHandler: @escaping RouteCompletionHandler) -> NavigationProviderRequest? {
        return calculate(options, completionHandler: completionHandler)
    }
    
    @discardableResult public func requestRoutes(options: MatchOptions, completionHandler: @escaping MatchCompletionHandler) -> NavigationProviderRequest? {
        return calculate(options, completionHandler: completionHandler)
    }
                     ...
                     completionHandler: { credentials, result in
                        ...
                        case .success(let routeRefreshResponse):
                            DispatchQueue.global().async {
                                let routeResponse = indexedRouteResponse.routeResponse.copy(with: routeOptions)
...
extension RouteResponse {
    func copy(with options: DirectionsOptions) -> RouteResponse {
        var copy = self
        copy.routes = copy.routes?.map { $0.copy(with: options) }
        return copy
    }
}

extension Route {
    func copy(with options: DirectionsOptions) -> Route {
